...
# Download the files from NOAA 
# (http://www1.ncdc.noaa.gov/pub/data/swdi/stormevents/csvfiles/)
# The batch download step here, is adapted from 
...
# Get the <a> nodes. Basically a list of files in the download site
a_nodes <- getNodeSet(site,"//a")

# Select the files of interest. In this case the files are those with the details of the
# storms
# There is one file per year, starting in 1951 and ending in 2015. 
...
# Make the full url for each file. Change the separator acordingly, keep the last '/' in the url for 
...
# Gunzip the files in the current working dir
mapply(function(x) gunzip(x),filenames)

# Rename files to make if easier to load later.
...
New_Names = gsub('_c[0-9]{8}','_3_2016', New_Names) # Here the "_3_2016" is the date the files were downloaded
...
# Combine all data frames from the list storms into one data.frame
# dplyr and plyr 
all_storms = ldply(storms, rbind)

# Writing the all_storms data.frame to a file (Again, the "3-16" is the date of download)
write.csv(all_storms, 'all_storms_3-16.csv')

# Creating a subset for tornados ONLY
...
write.csv(tornados, file = "tornados_3-16.csv")



########################### NO NEED FOR DOWLOADING #######################
# At this point the data is in the computer and all data compiled into two files:
# all_storms.csv and tornados.csv, no need to dowload.
...
# The mistakes may change as the data is constantly cleaned by NOAA. 
...
# This command checks for the 4 records:
...
# It is possible that there are other error, I advice to run a View cnad check the data manually.
...
write.csv(tornados_clean, file = "tornados_clean_3-16.csv")
